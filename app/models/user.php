<?php
/* SVN FILE: $Id: user.php 734 2011-10-03 20:47:56Z compass $ */
...
/*  function hasDuplicateUsername($username) {
  ...
  function getCurrentLoggedInUser(){
    ...
    // seems redundent but have to associate role array as well.
    $tmpUser = $this->find('first', array('conditions' => array('User.id' => $user['id'])));
    $user['RoleId'] = $tmpUser['Role'][0]['RolesUser']['role_id'];
  ...
  function get($path) {
    ...
    // Must return entire array for DepartmentList request.
    if (is_int((stripos($path, "DepartmentList")))) {
      return $value;
    } else { 
	  return $value[0];
    }
  ...
  function getMyDeptCoursesList() {
	$model = Classregistry::init('DepartmentCourse');
	return $model->find('list', array('conditions' => array('dept_id' => self::get('DepartmentList')),
