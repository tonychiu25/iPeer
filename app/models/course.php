<?php
/* SVN FILE: $Id: course.php 734 2011-10-03 20:47:56Z compass $ */
...
class Course extends AppModel
{
  ...
  /**
   * Associate a course with a department.
   */
  function addDepartment($course_id, $dept_id) {
  	return $this->habtmAdd('Dept', $course_id, $dept_id);
  }
  ...
  /**
   * Used in user/index's ajax list to filter courses based on the user's access level.
   * Eg, math department admins will filter out all non-math courses.  Filter only applies
   * to department admins and instructors.
   * 
   * @param : $queryData : pre-query data.
   * refer to "http://book.cakephp.org/view/1049/beforeFind".
   */
  function beforeFind($queryData) {
  	...
  	switch ($user['RoleId']) {
  	  case 2 :
		$filteredList = $this->getFilterCourseList($user['id']);	// Called from parent::app_model
  	  	$queryData['conditions']['Course.id'] = $filteredList;
  	  	break;
  	  	
  	  case 3 :
  	  	/*$filteredList = $this->UserCourse->find('list', array('conditions' => 
 ...
 */
  ...
  function getCourseName($id) {
    $course = $this->find('first', array('conditions' => array('Course.id' => $id), 'callbacks' => false));
  ...
  function getCourseByInstructor($instructorId, $type = 'all') {
    ...
    return $this->find($type, array('conditions' => array('instructor_id' => $instructorId),
                                    'fields' => $fields,
                                    'recursive' => 0));
  }
