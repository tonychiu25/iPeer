<?php
/* SVN FILE: $Id: users_controller.php 734 2011-10-03 20:47:56Z compass $ */
...
class UsersController extends AppController
{
  ...
  var $NeatString;
  var $uses = array('Department', 'DepartmentUser' ,'User', 'UserEnrol', 'Personalize', 'Course', 'SysParameter', 'SysFunction','Role', 'Group');
...
//                 )
  ...
  function __processForm() {
    ...
    if ($this->data = $this->User->save($this->data)) {
      ...
      // Attach associated dept to user.
      $deptUser = $this->DepartmentUser->getDeptUserByUserDeptId('first', $this->data['User']['selected_dept'], $this->User->id);
      $deptUser['DepartmentUser']['dept_id'] = $this->data['User']['selected_dept'];
	  $deptUser['DepartmentUser']['user_id'] = $this->User->id;
	  if($this->DepartmentUser->save($deptUser)) { 
	    return true;
	  }
    } else {
...
//         /* No cache!! (undoc.) */ false );
  ...
  function add() {
   // $this->AccessControl->check('functions/user', 'create');
    ...
    if (!empty($course_id))  {
 	...
 	var_dump($this->data);
    $this->set('departmentList', $this->Department->getDepartmentList());
    $this->set('course_id', $course_id);
  ...
  function edit($id) {
    ...
    if(empty($this->data)) {
      $this->User->id = $id;
      $deptList = $this->DepartmentUser->find('list', array('conditions' => array('user_id' => $id), 'fields' => array('dept_id')));
    ...
    $this->set('departmentList', $this->Department->getDepartmentList());
    $this->set('selectedDept', $deptList);
  ...
  /*function edit($id)
  {
    ...
    function delete($id, $type = null)
    {
      ...
      $roles = $this->User->getRoles($id);
      $userDept = $this->DepartmentUser->getDeptUserByUser('first', $id);
      if(!$this->AccessControl->hasPermissionDoActionOnUserWithRoles('DeleteUser', $roles)) {
        ...
        if($this->User->delete($id) && $this->DepartmentUser->delete($userDept['DepartmentUser']['id'])) {
