...
class AccessControlComponent extends Object {
  ...
  /**
   * Check if a given user has permission to access a course based on his/her
   * access level.  Eg. math admins are only allowed to access courses under
   * the math dept.
   * 
   * @return : true if user is allowed to access the course; otherwise false.
   */
  function checkCourseAccessibility($courseId, $userId) {
  	/* Get user's accessible course id list */
  	//$accessibleCourseList = $this->Course->getFilterCourseList($userId);
  	$accessibleCourseList = User::getMyDeptCoursesList();
  	$accessible = false;
  	if (in_array($courseId, $accessibleCourseList)) {
  	  $accessible = true;
  	}
